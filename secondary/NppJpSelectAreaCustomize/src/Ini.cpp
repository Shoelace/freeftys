...
Ini::Ini() : 
	fileName(_T("JpSelectArea.ini")){
}
Ini::~Ini(){
}

int Ini::setIniPath(const TCHAR *path){
...
int Ini::checkIniFile(){
	...
	if ( _tfopen_s(&fp, this->fullPath, _T("r")) == 0)
		return 0;
...
int Ini::writeDate(const TCHAR *section, const TCHAR *key, float data){
	...
	return (int)WritePrivateProfileString(section, key, buf, this->fullPath);
}
...
int Ini::writeDate(const TCHAR *section, const TCHAR *key, const TCHAR *data){
	return (int)WritePrivateProfileString(section, key, data, this->fullPath);
}

///戻り値は読み込んだ文字数 error時はhttp://msdn.microsoft.com/ja-jp/library/cc429779.aspxを
int Ini::readDate(const TCHAR *section, const TCHAR *key, TCHAR *data, int dataSize){
	return (int)GetPrivateProfileString(section, key, L"", data, (DWORD)dataSize, this->fullPath);
}
int Ini::readDate(const TCHAR *section, const TCHAR *key, int *data){
	TCHAR buf[NUMDIGIT];
	int readSize;
	readSize = (int)GetPrivateProfileString(section, key, L"", buf, (DWORD)NUMDIGIT, this->fullPath);
