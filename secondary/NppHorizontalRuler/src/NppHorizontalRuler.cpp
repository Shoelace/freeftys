...
void onHorizontalRulerBeNotified(SCNotification *notifyCode)
{
	...
	case NPPN_READY:
		...
		if( mainHRuler.GetEnable() )
			SendMessage(nppData._nppHandle, NPPM_SETMENUITEMCHECK, funcItem[RULERVISIBLE]._cmdID, TRUE);
		else
			SendMessage(nppData._nppHandle, NPPM_SETMENUITEMCHECK, funcItem[RULERVISIBLE]._cmdID, FALSE);
...
BOOL CALLBACK OptionDlgProc(HWND hWndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch(uMsg)
	{
	case WM_INITDIALOG:
