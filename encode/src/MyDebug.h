...
#define MYDEBUG_H
	...
	public:
		...
		~__Time() { 
			fprintf(stderr,"ExecTime-----------------End\t %u Millisecond\n", clock() - startTime);
		}
		//キャストのオぺレータ
		operator bool() { return false; }
	};
	//ifの条件式内での宣言はif内のスコープでのみ有効 Timeが作成されてキャストオペレータが評価される
	#define _Time(...) \
		if( __Time time = __Time(fprintf(stderr, "ExecTime-----------------Start\n" ## __VA_ARGS__ )) ) {} else
	...
	inline void _StartConsole()
	{
		...
		//本来はcloseしないとまずいが
		//closeするとstderrでエラーが発生するのでdebug用と割り切ってcloseしない
		//fclose(fp);
		//freopen("CON", "r", stdin);
